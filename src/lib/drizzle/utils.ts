...
export function jsonAgg<Column extends AnyColumn>(
    ...
    const orderBy = opts?.orderBy ? sql` order by
    ${opts.orderBy}` : sql``
    ...
    const where = opts?.nullish ? sql`true` : sql`${column}
    is not null`
...
>(shape: TFields, opts?: Omit<QueryBuilderOpts, "distinct">) {
    const nullishWhere = opts?.nullish
        ? sql`true`
        : sql`${sql.join(
            Object.values(shape).map((value) => sql`${value}
            is not null`),
            sql` and `
        )}`
...
>(fields: TFields, options?: QueryBuilderOpts) {
    const jsonbBuildObject = sql`jsonb_build_object
    (
    ${sql.join(
...
export function caseWhen<TColumn extends Column>(
    ...
    return sql<TColumn["_"]["dataType"]>`case
    ${sql.join(chunks)}
    end`
}
...
export function compose<TColumn extends Column>(
    ...
    const chunks = columns.map((column) => sql`${column}
    ::text`)
